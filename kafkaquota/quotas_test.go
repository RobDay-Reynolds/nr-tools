...
var _ = Describe("Quotas", func() {
	Describe("GenerateQuotasForClients", func() {
		...
		It("calculates by multiplying the current average throughput by the given multiplier", func() {
			...
			Expect(quotas[0].HumanReadableQuota).To(Equal("40 B/s"))
		})
		...
		Context("when the clients have throughput above 1 kB/s but below 1 MB/s", func() {
			...
			It("rounds up quotas to the next kB", func() {
				Expect(quotas).To(HaveLen(1))
				Expect(quotas[0].ClientID).To(Equal("my-client"))
				Expect(quotas[0].HumanReadableQuota).To(Equal("10 kB/s"))
			})
		})
		...
		Context("when the clients have throughput above 1 MB/s", func() {
			...
			It("rounds up quotas to the next MB", func() {
				Expect(quotas).To(HaveLen(1))
				Expect(quotas[0].ClientID).To(Equal("my-client"))
				Expect(quotas[0].HumanReadableQuota).To(Equal("10 MB/s"))
			})
		})
		...
		Context("when the multiplier would return a value higher than 60 MB/s", func() {
			...
			It("returns a quota of 60 MB/s", func() {
				...
				Expect(quotas[0].HumanReadableQuota).To(Equal("60 MB/s"))
			})
		})
		...
		Context("when the max throughput is higher than the average times the multiplier", func() {
			...
			It("returns a quota equal to half of the multiplier times the max", func() {
				...
				Expect(quotas[0].HumanReadableQuota).To(Equal("3 kB/s"))
			})

			Context("when the quota would otherwise be above 60 MB/s", func() {
				BeforeEach(func() {
					averageThroughput = 10
		...
		Context("when there are multiple clients", func() {
			...
			It("returns quotas for each of them", func() {
				...
				Expect(quotas[1].HumanReadableQuota).To(Equal("60 MB/s"))

				Expect(quotas[2].ClientID).To(Equal("my-client-3"))
				Expect(quotas[2].Quota).To(Equal(62914560))
				Expect(quotas[2].HumanReadableQuota).To(Equal("60 MB/s"))
			})
		})
	})
})
